...
- [x] Exported full pipeline outputs (hybrid retrieval + recursive chunking, all 41 questions — retrieved chunks with rank/score/text, generated answer, citations, golden reference) to `eval/pipeline_outputs_for_external_judge.jsonl`, for external LLM-as-judge evaluation via Claude Sonnet 5 (independent of the Groq-family judge already in the DeepEval harness). Judging happens outside this repo; results to be folded back into `eval/results.md` once available.

Next: get explicit go-ahead before continuing further Phase 3 experiments (contextual chunk headers) or moving to Phase 4. Once the external judge results come back, fold them into `eval/results.md` and decide whether to promote recursive chunking to the default.
