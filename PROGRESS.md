...
Next: get explicit go-ahead before continuing. Other open items: decide on a fix for the GDPR retrieval gap (query reformulation/HyDE, metadata-filtered secondary GDPR pass, or just excluding the mirror-page junk chunks); investigate the new paragraph-38 chunk-boundary gap in the Article 50 transparency guidelines (Step 10); run recursive chunking through a same-config comparison; strip Service Desk boilerplate (Step 6/8 finding); add a chunk-level recall metric to the automated harness; consider a self-consistency check across paraphrased queries; or move to Phase 4.
