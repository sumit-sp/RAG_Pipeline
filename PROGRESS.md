...
- [ ] **Hard-subset external judging, in progress.** Per user direction, judging the contextual-headers pipeline on a targeted 15-question hard subset (8 cross-reference, 3 multi-hop, 2 temporal-conflict, 2 single-hop questions that miss the automated retrieval-hit check) rather than the full 41 — same JSON-export + Sonnet-5-external-judging pattern as the contextual headers themselves. `eval/export_hard_subset_for_external_judge.py` ran the live pipeline (hybrid retrieval + contextual headers, one Groq generation call per question — allowed, generation isn't restricted) and wrote `eval/hard_subset_outputs_for_external_judge.jsonl` (15 records). Rubric + output format for the user's Sonnet-5 session: `eval/HARD_SUBSET_JUDGE_INSTRUCTIONS.md` — adds `chunk_level_hit` and `citation_accuracy` to the prior rubric (Retrieval Hit, MRR, Faithfulness, Answer Correctness, Outcome Label) to specifically target the still-open Step-6 findings (document-level hit-rate masking chunk-level misses; citations echoing the retrieved set rather than reflecting real support). Awaiting `eval/hard_subset_judge_results.jsonl` back from the user.
...
Next: get explicit go-ahead before continuing. Other open items: run recursive chunking through a same-config comparison; act on the Step-6 findings beyond what the hard-subset judging above already targets (strip Service Desk boilerplate); or move to Phase 4.
