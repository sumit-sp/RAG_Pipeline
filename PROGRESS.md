...
- [ ] **Full 41-question external judging, in progress.** Per user direction, extending the contextual-headers judging from the 15-question hard subset (Step 8) to all 41 questions, for a complete comparable picture. `eval/export_contextual_headers_full_for_external_judge.py` ran the live pipeline (hybrid retrieval + contextual headers) and wrote `eval/contextual_headers_full_outputs_for_external_judge.jsonl` (41 records). Rubric: `eval/CONTEXTUAL_HEADERS_FULL_JUDGE_INSTRUCTIONS.md` (same 6-metric rubric as Step 8, for continuity). Awaiting `eval/contextual_headers_full_judge_scores.jsonl` back from the user.

Next: get explicit go-ahead before continuing. Other open items: decide on a fix for the GDPR retrieval gap (query reformulation/HyDE, metadata-filtered secondary GDPR pass, or just excluding the mirror-page junk chunks); run recursive chunking through a same-config comparison; strip Service Desk boilerplate (Step 6/8 finding); add a chunk-level recall metric to the automated harness; consider a self-consistency check across paraphrased queries; or move to Phase 4.
